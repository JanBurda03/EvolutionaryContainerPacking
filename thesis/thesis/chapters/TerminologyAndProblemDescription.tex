...
\subsection{Datasets}
For the experimental evaluation, a dataset consisting of 120 problem instances inspired by \textcite{martello2000} was generated. 
The dataset is divided into four classes, each containing 30 instances. 
For every class, 10 instances were generated for each value of $n \in \{50, 100, 150\}$. The container dimensions are identical for all instances and are defined as $X = Y = Z = 100$.

The size limits of the boxes differ for each class and are defined as follows:

\begin{itemize}
    \item Class 1: $x \in [1, X/2], \; y \in [Y/2, Y], \; z \in [Z/2, Z]$
    \item Class 2: $x \in [1, X/2], \; y \in [1, Y/2], \; z \in [Z/2, Z]$
    \item Class 3: $x \in [1, X/2], \; y \in [1, Y/2], \; z \in [1, Z/2]$
    \item Class 4: $x \in [1, X], \; y \in [1, Y], \; z \in [1, Z]$
\end{itemize}
...
For Classes 1–3, the boxes are generated randomly with a probability of 60\,\% from the corresponding class and 20\,\% from each of the remaining two classes. This approach preserves the main structural characteristics of each class while introducing additional variability.
...
In the original dataset, the weight of every box is set to 1. 
The maximum allowed weight of each container is set to X*Y*Z, corresponding to the weight of a fully filled container with unit density. In practice, this means the weight constraint is never restrictive, as any container can accommodate all boxes by weight.
