...
To run the application without arguments, the following command can be used from the root directory of the repository:

\begin{verbatim}
src/App/bin/Release/net8.0-windows/App.exe
\end{verbatim}

The application can also be executed with a configuration file.
An example configuration file is available in the \texttt{examples/input} directory:

\begin{verbatim}
src/App/bin/Release/net8.0-windows/App.exe \
examples/input/ProgramSettingExample.json
\end{verbatim}
...
\begin{itemize}
    ...
    \item \texttt{UseIndividualsAsRelative} -- indicates whether \texttt{Individuals} is\\interpreted as a relative value; if \texttt{true}, the population size is multiplied by $N$,
...
\section{Python Scripts and Dependencies}

The repository also contains Python scripts in the \texttt{scripts} directory. 
These scripts are used for dataset generation, running experiments, and result visualization, 
and are described in more detail in Section~\ref{app:scripts}.

To run these scripts, Python (version 3.10 or newer) must be installed. 
All required dependencies are listed in the \texttt{requirements.txt} file located in the \texttt{scripts} directory.

The dependencies can be installed using the following command from the root directory of the repository:
